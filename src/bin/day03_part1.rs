...
fn parse_input(input: String) -> (Vec<Seg>, Vec<Seg>) {
    ...
    for p in input.split(',') {
        let v0 = last_vert.clone();
        let dir = p.as_bytes()[0usize] as char;
        let dist: i32 = p[1..].parse().unwrap();
        match dir {
            'U' => {last_vert.y += dist; vertical.push(Seg::new(v0, last_vert))},
            'D' => {last_vert.y -= dist; vertical.push(Seg::new(last_vert, v0))},
            'L' => {last_vert.x -= dist; horizont.push(Seg::new(last_vert, v0))},
            'R' => {last_vert.x += dist; horizont.push(Seg::new(v0, last_vert))},
            _ => panic!("Bad input"),
        };
    }
    (vertical, horizont)
