...
impl Instruction {
	pub const fn new(exec: &'static dyn Fn(&mut Machine), nb_params: usize, arg_store: usize, name: &'static str) -> Instruction {
		Instruction {
			exec,
			nb_params,
			arg_store,
...
impl Machine {
	...
	pub fn run_one(&mut self) {
		...
		if opcode >= INSTRUCTIONS.len() || opcode == 0 {
			match opcode == 99 {
				true => { println!("{:?}", self.ram); std::process::exit(0); },
				false => panic!("ERROR: opcode is {}\n{:?}", opcode, self.ram),
			}
		}
		let inst = &INSTRUCTIONS[opcode];
		const FETCH_MODES: [usize; 5] = [100, 1000, 10000, 100000, 1000000];
		...
		println!("{} reg: {:?}", inst.name, &self.reg[..inst.nb_params as usize]);
		self.pc += 1 + inst.nb_params as usize;
		(inst.exec)(self);
		println!("pc move to {}", self.pc);
	}
}
